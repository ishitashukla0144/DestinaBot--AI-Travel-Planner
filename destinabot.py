...
def generate_itinerary(from_location, to_location, days, month, moods, transport, budget_type):
    ...
    messages = [
        {"role": "system",
         "content": "You are an expert travel guide who provides structured and detailed travel itineraries."},
        {"role": "user",
         "content": f'''Generate a detailed {days}-day travel itinerary from {from_location} to {to_location} in {month}.
        Preferences:
        - Moods: {mood_str}
...
def main():
    if st.session_state["page"] == "home":
        ...
        # Show download button if itinerary exists
